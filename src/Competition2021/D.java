...
public class D {
    public static void main(String[] args) {
        ...
        while (t-->0){
            ...
            Line[]arr=new Line[n];
            ArrayList<Point> list=new ArrayList<>(n*(n-1));
            while (n-->0){
                String[] s=scan.nextLine().split(" ");
                arr[n]=new Line(Double.parseDouble(s[0]),Double.parseDouble(s[1]));
            }
            n=arr.length;
            for(int i=0;i<n;i++)
                for (int j=0;j<n;j++)
                    if (i!=j){
                        Point x=arr[i].intersect(arr[j]);
                        if (x!=null)
                            list.add(x);}
            Stack<Point> points=new Stack<>();
            list.sort(Point.R_ORDER);
...
//y=mx+q
    ...
    Point intersect(Line line){
        ...
        double x=(this.q-line.q)/(line.m-this.m);
        double y=getY(x);
